...
function initMap() {
  ...
  function MapMarker(name, category, lat, lng) {
    ...
    //currently not in use but might be handy in the future
  ...
  //Init ViewModel defined above
  vm = new MarkerViewModel();
  ko.applyBindings(vm);
}
function googleError() {window.alert("Failed to load Map API, pls reload.");}
function knockoutError() {window.alert("Failed to load  knockout, pls reload.");}